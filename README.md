...
```powershell
uv run directed-foraging <subcommand>
```

for a list of all sub commands available:

```powershell
uv run directed-foraging -h
```
...
To manage experiments and input files, this repository contains a launcher script that can be used to run the task. This script is located at `./src/aind_behavior_directed_foraging/launcher.py`. It can be run from the command line as follows:

```powershell
uv run directed-foraging clabe
```

Additional arguments can be passed to the script as needed:

```powershell
uv run directed-foraging clabe -h
```
...
```powershell
uv run directed-foraging data-qc <path-to-data-dir>
```
...
```powershell
uv run directed-foraging data-mapper --data-path <path-to-data-dir>
```
...
```powershell
uv run directed-foraging regenerate
```
